  ...
  const [active, setActive] = useState<number>()
    ...
    <div className='max-md:mx-5 mx-20 ' >
      ...
      <SwiperSlide key={data.id} >
           ...
           <div className='' ><Image width={50} height={20} src={data.user_pic} alt={data.user} className=" mx-auto rounded-full w-20 h-20 object-cover " /></div>
           <div className="user_name max-sm:text-lg text-2xl text-emerald-700 font-semibold my-4 ">{data.user}</div>
           <div className="rewiew max-sm:text-xs text-sm text-gray-600 max-w-[80%] mx-auto italic ">"{data.review}"</div>
           </div>
        </SwiperSlide>
        ))}
